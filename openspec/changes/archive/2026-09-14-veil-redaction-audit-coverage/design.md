...
- Responses 审计覆盖缺口：`src/handler/llm/pump/fragments.rs:181-372` Responses 分支仅处理 `function_call_arguments`/检索/`output_item`；`src/handler/llm/pump/event.rs:258-266` `is_minor_event` 将 `["reasoning","mcp","code_interpreter","image_gen"]` 全按次要透传；Python `_llm.py:783-791` `kind_map` 将四类 delta 全部映射 `function_call_arguments`（`RED-4`）。
...
- Chat 分桶：`src/handler/llm/pump/fragments.rs:32,45` 以 `choices.iter().enumerate()` 的位置 `ci` 参与 `chat_bucket(ci, idx)`（`src/service/llm_gateway/tool.rs:167`）（`RED-7`）。
