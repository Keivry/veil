...
- **RED-7（P2）Chat 审计分桶用位置 `ci` 而非声明 `choices[].index`**：`src/service/llm_gateway/tool.rs:143-147` 以 `choices.iter().enumerate()` 的位置 `ci` 参与 `chat_bucket(ci, idx)`（`src/service/llm_gateway/tool.rs:167`），乱序/跳号 `choices[].index` 时槽归属错位，审计对象与实际 choice 不匹配。
