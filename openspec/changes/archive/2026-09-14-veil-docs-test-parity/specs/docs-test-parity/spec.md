...
README 与 canonical spec 中指向 Python 原仓 `_llm.py` 的每个行号引用 SHALL 与该文件当前内容语义匹配；引用 502 响应体形态时 SHALL 指向实际构造该体的行（`_llm.py:2951-2961`），引用「超限观测（warning + `metrics_ctx['status']=502`）」时 SHALL 覆盖 `metrics_ctx['status']=502` 所在行（`2936-2946`）。已核实准确的行号引用 SHALL 保持不变；指向冻结归档语料的引用 SHALL NOT 被回改。
...
仓库顶层模块文件 SHALL 具备 `//!` 模块级文档；`src/router.rs` SHALL 包含描述路由装配与中间件职责的 `//!` 头，与其它模块风格一致。
