...
- **出站响应保真**：`T2` 非流对话路径丢弃上游全部响应头（`src/handler/llm/nonstream.rs:253` 的 `(StatusCode, String).into_response()` 被 axum 强制 `text/plain; charset=utf-8`、`:275-277` 的字节体被强制 `application/octet-stream`），`content-type`/`Retry-After`/`x-request-id`/rate-limit 头全丢；`T6` 非流还原用字节级 `restore_response_with_spans`（`:231`）而流式用 JSON 转义变体（`src/service/redaction/scope.rs:194`、`src/handler/llm/pump/spawn/event_loop.rs:576-584`），vault 明文含 `"`/`\` 时写破 JSON 后整响应回退未还原原文；`T10` 错误/超限/空体响应缺 `x-veil-protocol`（仅 JSON 后处理分支置位）。
