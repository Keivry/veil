...
- **`D2`（LOW，死码）`CallerRegistry::migrate_python_registry`**：`src/registry.rs:372` 为 `pub fn`，生产零调用（`load_from` 为生产唯一加载入口），仅测试 `:644` 引用（含 `.bak` 备份断言）。要求移入 `#[cfg(test)]` 或 `examples/`。
