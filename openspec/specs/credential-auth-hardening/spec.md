# credential-auth-hardening Specification
...
- **WHEN** 核查本要求的实现定位引用
- **THEN** 引用为符号锚点 `src/service/credential/vault_ops.rs::emergency_revoke`（可解析），不再为行号形式
