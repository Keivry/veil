...
请求/响应体 SHALL 经 JSON-aware 叶级脱敏还原（含嵌套 stringified JSON 与 BOM），失败回退原串；残缺与幻觉 token SHALL 在出口剥离；Chat SHALL 以唯一裸 `data: [DONE]` 收尾，Anthropic SHALL 按三态收尾（审计阻断 → 五件套有序 `message_start` → `content_block_start` → `content_block_stop` → `message_delta` → `message_stop`；真空流（零有效分片）→ 最小二帧 `message_start` + `message_stop`；正常上游结束 → 不合成任何帧，透传原始终端），Responses 以 `completed/failed` 收尾；空流 SHALL 按协议注入最小可解析帧，`502/401` 透传；审计阻断 SHALL 按协议形态注入且不泄漏参数明文。
...
- **THEN** 下游依序收到 `message_start` → `content_block_start` → `content_block_stop` → `message_delta` → `message_stop`，首帧为 `message_start`，不缺帧、不乱序；真空流（零有效分片）改收最小二帧 `message_start` + `message_stop`；正常上游结束不合成任何帧
