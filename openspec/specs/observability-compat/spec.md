# observability-compat Specification

## Purpose
修复管理面与指标查询的兼容断裂：兼容旧 `range/metrics/verdict` 查询或显式 BREAKING 声明新口径并升级调用方，同时明确 JSON 索引终态、鉴权优先级、TCP 限流，以及 WAL、`busy_timeout`、`0600` 与磁盘满载降级等存储语义。
