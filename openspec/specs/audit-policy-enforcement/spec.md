...
系统 SHALL 将裸 `curl`/`wget` 外传目标（含 URL 形态或输出重定向形态）识别为网络外传，SHALL NOT 仅依赖管道进解释器或携带 `--data`/`-d`/`--post-data` 才判定。规则匹配 SHALL 使用命令词边界以限制误报。命中外部 host 时 SHALL 拦截，命中内网后缀时 SHALL 按既有内网豁免口径放行。
