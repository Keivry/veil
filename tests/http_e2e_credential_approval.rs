...
use {
    common::{serve, test_app_cfg},
    std::time::{Duration, Instant},
    veil::{service::matrix::ResolveOutcome, state::AppState},
};
...
async fn default_202_pending_returns_immediately_with_ticket() {
    let (app, state) = test_app_cfg(&[("AUTO_APPROVE", "none")], |_| {});
...
async fn block_wait_approve_returns_credential_same_request() {
    let (app, state) = test_app_cfg(&blocking_env(), |_| {});
    let (base, handle) = serve(app).await;
    let client = reqwest::Client::builder()
...
async fn block_wait_timeout_rejects_without_hanging() {
    let (app, state) = test_app_cfg(&blocking_env(), |cfg| {
